...
public final class HudOverlay {
    public static final int BASE_WIDTH = 258;
    ...
    private static void renderInternal(DrawContext context, boolean editing) {
        MinecraftClient client = MinecraftClient.getInstance();
        if (client.options.hudHidden || client.player == null || !RaidMineStaffMod.config().hudEnabled) return;

        SessionStats stats = RaidMineStaffMod.stats();
        stats.observeConnection(client);
    ...
    private static void renderStatsBar(DrawContext context, Layout l, SessionStats stats, boolean editing) {
        UiTheme.shadow(context, l.x(), l.y(), l.width(), l.height(), Math.max(6, scale(9, l.scale())));
        UiTheme.roundedRect(context, l.x(), l.y(), l.width(), l.height(), Math.max(6, scale(9, l.scale())),
                UiTheme.argb(230, 11, 14, 22));
        UiTheme.roundedRect(context, l.x() + 1, l.y() + 1, l.width() - 2, l.height() - 2,
                Math.max(5, scale(8, l.scale())), UiTheme.argb(244, 22, 27, 38));
        context.fillGradient(l.x() + scale(10, l.scale()), l.y() + 1,
                l.x() + l.width() - scale(10, l.scale()), l.y() + Math.max(2, scale(2, l.scale())),
        ...
        cursor += logoW + scale(3, l.scale());
        UiTheme.text(context, tr, "RM Tools", cursor, l.y() + scale(7, l.scale()),
                Math.max(8F, 9.5F * l.scale()), UiTheme.TEXT, true);
        cursor += scale(48, l.scale());
        ...
        UiTheme.text(context, tr, formatTime(stats.elapsedSeconds()), cursor,
                l.y() + scale(7, l.scale()), Math.max(8F, 9.0F * l.scale()), UiTheme.TEXT, true);

        if (editing) {
            UiTheme.glow(context, l.x() - 2, l.y() - 2, l.width() + 4, l.height() + 4,
                    Math.max(7, scale(10, l.scale())), UiTheme.accent());
            Rect handle = l.resizeHandle();
            UiTheme.roundedRect(context, handle.x(), handle.y(), handle.w(), handle.h(), 5, UiTheme.accent());
    ...
    private static void renderNoticeBar(DrawContext context, Layout l, UiNotificationCenter.Notice notice) {
        ...
        int width = Math.max(scale(146, l.scale()), Math.round(l.width() * p));
        int x = l.x() + (l.width() - width) / 2;
        int y = l.y() - Math.round((1F - p) * scale(8, l.scale()));
        int alpha = Math.round(245F * p);
        int accent = notice.kind() == UiNotificationCenter.Kind.VIOLATION ? UiTheme.accent() : UiTheme.SUCCESS;

        UiTheme.glow(context, x, y, width, l.height(), Math.max(7, scale(9, l.scale())), accent);
        UiTheme.roundedRect(context, x, y, width, l.height(), Math.max(7, scale(9, l.scale())),
                UiTheme.withAlpha(UiTheme.PANEL_2, alpha));
        context.fillGradient(x + scale(8, l.scale()), y + 1,
                x + width - scale(8, l.scale()), y + Math.max(2, scale(2, l.scale())),
        ...
        UiTheme.icon(context,
                notice.kind() == UiNotificationCenter.Kind.VIOLATION ? UiIcon.WARN : UiIcon.BELL,
    ...
    private static void renderMention(DrawContext context) {
        UiNotificationCenter.Notice notice = UiNotificationCenter.mention();
        if (notice == null) return;
        MinecraftClient client = MinecraftClient.getInstance();
        float p = UiNotificationCenter.progress(notice);
        int width = Math.min(270, client.getWindow().getScaledWidth() - 24);
        int height = 36;
        int x = (client.getWindow().getScaledWidth() - width) / 2;
        int baseY = client.getWindow().getScaledHeight() - 72;
        int y = baseY + Math.round((1F - p) * 18F);
        int alpha = Math.round(238F * p);

        UiTheme.glow(context, x, y, width, height, 12, UiTheme.accent());
        UiTheme.roundedRect(context, x, y, width, height, 12, UiTheme.withAlpha(UiTheme.PANEL_2, alpha));
        UiTheme.roundedRect(context, x + 9, y + 9, 18, 18, 7, UiTheme.withAlpha(UiTheme.accent(), alpha));
        UiTheme.icon(context, UiIcon.BELL, x + 12, y + 12, 12, UiTheme.withAlpha(UiTheme.TEXT, alpha));
        UiTheme.text(context, client.textRenderer, notice.title(), x + 36, y + 7,
                10.2F, UiTheme.withAlpha(UiTheme.TEXT, alpha), true);
        UiTheme.text(context, client.textRenderer, notice.message(), x + 36, y + 21,
                8.6F, UiTheme.withAlpha(UiTheme.MUTED, alpha), false);
    }
